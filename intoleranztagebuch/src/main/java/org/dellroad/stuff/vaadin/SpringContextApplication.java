...
/**
 * Vaadin application implementation that manages an associated Spring
 * {@link WebApplicationContext}.
 * 
 * <h3>Overview</h3>
 * 
 * <p>
 * Each Vaadin application instance is given its own Spring application context,
 * and all such application contexts share the same parent context, which is the
 * one associated with the overal servlet web context (i.e., the one created by
 * Spring's {@link org.springframework.web.context.ContextLoaderListener
 * ContextLoaderListener}). A context is created when a new Vaadin application
 * instance is initialized, and destroyed when it is closed.
 * </p>
 * 
 * <p>
 * This setup is analogous to how Spring's
 * {@link org.springframework.web.servlet.DispatcherServlet DispatcherServlet}
 * creates per-servlet application contexts that are children of the overall
 * servlet web context.
 * </p>
 * 
 ...
 * 
 * <p>
 * For each Vaadin application {@code com.example.FooApplication} that
 * subclasses this class, there should exist an XML file named
 * {@code FooApplication.xml} in the {@code WEB-INF/} directory that defines the
 * per-Vaadin application Spring application context. This naming scheme
 * {@linkplain #getApplicationName can be overriden}, or explicit config file
 * location(s) can be specified by setting the
 * {@link #VAADIN_CONTEXT_LOCATION_PARAMETER} servlet parameter.
 * </p>
 * 
 ...
 * 
 * <p>
 * It is also possible to configure beans outside of this application context
 * using AOP, so that any invocation of {@code new FooBar()}, where the class
 * {@code FooBar} is marked {@link VaadinConfigurable @VaadinConfigurable}, will
 * automagically cause the new {@code FooBar} object to be configured by the
 * application context associated with the {@linkplain ContextApplication#get()
 * currently running application instance}. In effect, this does for Vaadin
 * application beans what Spring's
 * {@link org.springframework.beans.factory.annotation.Configurable
 * @Configurable} does for regular beans.
 * </p>
 * 
 * <p>
 * Note however that Spring
 * {@linkplain org.springframework.beans.factory.DisposableBean#destroy destroy
 * methods} will not be invoked on application close for these beans, since
 * their lifecycle is controlled outside of the Spring application context (this
 * is also the case with
 * {@link org.springframework.beans.factory.annotation.Configurable
 * @Configurable} beans). Instead, these beans can register as a
 * {@link ContextApplication.CloseListener} for shutdown notification.
 * </p>
 * 
 * <p>
 * For the this annotation to do anything, {@link VaadinConfigurable
 * @VaadinConfigurable} classes must be woven (either at build time or runtime)
 * using the <a
 * href="http://www.eclipse.org/aspectj/doc/released/faq.php#compiler">AspectJ
 * compiler</a> with the {@code VaadinConfigurableAspect} aspect (included in
 * the <code>dellroad-stuff</code> JAR file).
 * </p>
 * 
 * @see ContextApplication#get
 * @see <a
 *      href="https://github.com/archiecobbs/dellroad-stuff-vaadin-spring-demo3">Example
 *      Code on GitHub</a>
 */
...
public abstract class SpringContextApplication extends ContextApplication {
	...
	/**
	 * Optional servlet initialization parameter (
	 * <code>{@value #VAADIN_CONTEXT_LOCATION_PARAMETER}</code>) used to specify
	 * the location(s) of the application context XML file(s). Multiple XML
	 * files may be separated by whitespace. If this parameter is not defined,
	 * then the XML file location is built using {@code /WEB-INF/} as a prefix,
	 * the string from {@link #getApplicationName}, and {@code .xml} as a
	 * suffix.
	 */
	 ...
	 * 
	 * <p>
	 * Works just like {@link ContextApplication#get()} but returns this
	 * narrower type.
	 * </p>
	 * 
	 ...
	 * 
	 * <p>
	 * After initializing the associated Spring application context, this method
	 * delegates to {@link #initSpringApplication}.
	 */
	@Override
	...
	/**
	 * Perform any application-specific shutdown work. This will be invoked at
	 * shutdown after this Vaadin application and the associated
	 ...
	 * application is closed. In such cases overriding this method is not
	 * necessary.
	 * </p>
	 * 
	...
	/**
	 * Post-process the given {@link WebApplicationContext} after initial
	 * creation but before the initial
	 ...
	 * 
	 * @param context
	 *            the associated {@link WebApplicationContext} just refreshed
	 * @see #onRefresh
	...
	/**
	 * Perform any application-specific work after a successful application
	 * context refresh.
	 * 
	 * <p>
	 * The implementation in {@link SpringContextApplication} does nothing.
	...
	/**
	 * Get the name for this application. This is used as the name of the XML
	 * file in {@code WEB-INF/} that defines the Spring application context
	 * associated with this instance, unless the
	...
	private void loadContext() {
		...
		// Logging
		this.log.info("loading application context for Vaadin application "
		...
		String configLocationValue = this
				.getProperty(VAADIN_CONTEXT_LOCATION_PARAMETER);
		...
		this.context.addApplicationListener(new SourceFilteringListener(
				this.context, new RefreshListener()));
		...
		// Get notified of application shutdown so we can shut down the context
		// as well
